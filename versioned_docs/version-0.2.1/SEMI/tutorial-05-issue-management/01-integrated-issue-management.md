...
![003](./img/003.png)
1. 이슈에 대한 그래프입니다. 그래프 종류는 다음과 같습니다. 
    - **원형 그래프** : 이슈 진행 현황, 이슈 발생 구분, 귀책 부서
    - **막대 그래프** : 불량 유형별 현황, 불량 원인별 현황, 개선 대책 유형별 현황
...
1. 품질 이슈 목록입니다.
    ...
    - **이슈 기본 정보** : 이슈 No., 중요도, 상태, 이슈 발생 장소(대분류, 소분류), Pjt No. (WBS No.), 발생 구분, 이슈 발생일자, 이슈 등록일자
    - **설비 관련 정보** : 설비코드(고객사), 공정명, Module
    - **이슈 상세 정보** : 이슈 현상, 이슈 구분, 추정 원인, 이슈 현상분석(추정원인)
    - **이슈 등록 정보** : 이슈 등록부서, 이슈 등록자, 등록의견
    - **관련 부품 정보** : 부품명, 부품코드, 부품 S/N, 수량
    - **조치 관련 정보** : 조치 담당 부서, 조치 담당자, 조치 요청 납기, 장비 조치 여부, 장비 조치 일자
    - **대책 관련 정보** : 재발 방지 대책 조치 여부, 대책 수립 완료일, 조치 완료일, Lead Time, EcrNo, 불량 유형, 불량 원인, 재발 여부, 재발방지대책(대분류, 소분류)
    :::
